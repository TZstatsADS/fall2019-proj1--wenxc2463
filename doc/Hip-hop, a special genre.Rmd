...
#### In this R notebook, I try to find the features of the hip-hop song lyrics through Q&A.
...
First, I do the data processing and saved data in a R file. I remove all the stopwords, stem words and convert them to a tidy object. 
...
Then Let's take a look at the dataset at first, the number of rock and pop songs accounts for over 60 percent of all songs. Hence, in addition to the hip-hop songs, I mainly study the rock and pop songs.
```{r}
library(plotly)
dt_lyrics_genre_count <- dt_lyrics %>% group_by(genre) %>% count(genre) %>% ungroup() %>% mutate(proportion = n / sum(n))
library(ggplot2)
...
## Q1: What are the most frequently used words in hip-pop lyrics? What about rock and pop?
...
## Q2: What are the emotions of hip-pop songs? 
...
In fact, I found that in most of hip-hop songs, there are few duplicate words. The most commonly used word in one song mainly accounts for less than 10 percent of the whole song. In addtion, in hip-hop lyrics, it seems that there is hardly any songs in which the most frequently used word accounts for more than 25%. Surprisingly, it seems that there are more pop songs and rocks songs in which the most commonly used word accounts for more than 15 percent.

Actually, there are indeed some hip-hop songs with many duplicate words. For example, There are 195 'shake' in the song 'suga-baby' and there are 170 'white' in the 'what-if-i-was-white'. It is likely that these songs belong to a specific type of hip-hop music.
```{r}
hip_hop_word_repetition <- hip_hop_tibble %>%
  group_by(id) %>% count(word, sort = TRUE)
...
3. Most Hip-Hop songs do not use words repeatedly. There are more rock and pop songs in which the most frequently used words account for more than 15%.
